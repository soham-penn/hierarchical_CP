#!/usr/bin/env python3
"""GHCP merger-weight grid on the paper Poi(25) latent-intercept DGP (App. D.4).

Each replicate is drawn once; GHCP is evaluated at six merger weights
λ_local ∈ {1/7,...,6/7} (code stores the complementary global weight
w_g = 1-λ_local via set_w_g_override). The global RF, donor draw, and
quantile seed are shared; only the blend

    μ̃ = (1 − λ_local) μ̂_global + λ_local Ȳ_local

changes.

Paper DGP: γ=5, K=20, N_j ~ Poi(25) (reject 0), RF 50 trees / min leaf 5,
α=0.1, o ∈ {0,5,10,15,20}, target index 35, η=0.5, deterministic quantiles.
"""
...
from methods.mu_methods import create_mu_method_random_forest_offset, set_w_g_override
from scores import make_quantile_seed

# Global-weight grid passed to set_w_g_override: w_g = k/7 for k=1..6
# ⇒ λ_local = 1 - w_g ∈ {6/7,...,1/7} (same six values; figures label λ_local).
WEIGHT_GRID = tuple(k / 7.0 for k in range(1, 7))
RF_NTREE = 50
RF_NODESIZE = 5
RF_RANDOM_STATE = 123
RESULTS_PREFIX = "effect_of_weights"
    ...
    *,
    ...
    for o in o_values:
        ...
        q_seed = make_quantile_seed(
            quantile_base_seed, experiment_id, target_index, o, "donor_hcp_wgrid"
        )
        # One vector of GHCP estimators on this (replicate, o) draw.
        for w_g in weights:
...
def run_experiment(config: dict, n_workers: int, chunk_size: int) -> pd.DataFrame:
    ...
    print("=" * 96)
    print("EFFECT OF WEIGHTS  |  Poi(25) latent intercept RF")
    print("=" * 96)
    ...
    manifest = {
        ...
        "weight_meaning": "w_g = weight on global RF; w_local = 1-w_g > 0",
        "quantile_mode": config.get("quantile_mode", "deterministic"),
        "base_seed": BASE_SEED,

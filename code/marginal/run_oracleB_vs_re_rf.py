...
-------
oracle_B
    Center = m(u,x) + B (full conditional mean). No within-group merging.
    This is the best possible predictor when B is known; GHCP widths should
    sit near the Gaussian oracle EE[2 z_{1-α/2} sqrt(σ²(U))] ≈ 4.22 at α=0.1.

rf_re
    RF global + local Ȳ blend with RE weights
    w_g = (σ̂²/τ) / (τ̂_B² + σ̂²/τ) estimated by leave-one-group-out MoM.

rf_sqrt
    Same RF + Ȳ blend with legacy sqrt merger (c=0.5), for reference.
"""
...
from methods.mu_methods import (
    create_mu_method_oracle_yxub,
    register_oracle_group_B,
)
from scores import make_quantile_seed

RESULTS_PREFIX = "oracleB_vs_re_rf"
RF_NTREE = 50
RF_NODESIZE = 5
RF_RANDOM_STATE = 123
N_WORKERS_DEFAULT = 8
...
def _run_one(
    *,
    ...
    # Register B for all groups (oracle knows every group's latent intercept).
    mu_oracle["_b_by_u"].clear()
    register_oracle_group_B(mu_oracle, U_cal, Z_cal)
    register_oracle_group_B(mu_oracle, U_test, [Z_test], group_index_vector=[0])

    rows = []
    variants = {
        "oracle_B": (mu_oracle, 0),          # tau_override=0: no merge
        "rf_re": (mu_rf_re, None),
        "rf_sqrt": (mu_rf_sqrt, None),
    }
    for o in o_values:
        row = {
        ...
        for key, (mu_m, tau_ov) in variants.items():
            res = compute_donor_hcp_randomized_interval(
                ...
                return_intermediates=(key == "rf_re"),
            )
            lo, hi = res["interval"]
            w = _width(lo, hi)
            row[f"coverage_{key}"] = float(lo <= true_target <= hi)
            row[f"width_{key}"] = w
            row[f"infinite_{key}"] = int(not np.isfinite(w))
            if key == "rf_re":
...
def run_chunk(worker_id, number_experiments_chunk, experiment_offset, config):
    ...
    mu_oracle = create_mu_method_oracle_yxub(rho=rho)
    ...
    mu_rf_sqrt = create_mu_method_random_forest_offset(
...
def run_experiment(config_name: str, config: dict, n_workers: int) -> pd.DataFrame:
    ...
    print("=" * 96)
    print(f"ORACLE B vs RF+RE: {config_name}")
    print("=" * 96)
    print(f"gamma={config['gamma']}  alpha={config['alpha']}  B={config['total_replicates']}")
    print(f"o_values={config['o_values']}")
    print("methods: oracle_B (m(u,x)+B, no merge) | rf_re (variance weights) | rf_sqrt")
    print("=" * 96)

...
Reuses stored seeds from paper-results/acs/seeds_manifest.json:
...
Std-CP is local RF on a random half of the first o rows in the (permuted)
target PUMA; conformal scores on the other half; studentized |Y-μ|/σ.

Does not re-run GHCP/HCP. Patches Std-CP rows into the finished detailed CSVs
and regenerates summaries + paper plots.
"""
...
def _load_cohort():
    """Match run_acs_yoep_fb_min21_permute paper cohort filters."""
    acs._set_target_index(TARGET_INDEX)
    ...
    X = build_design_matrix_acs(
        df, exclude_entry_recency=True, exclude_cow=True
    )
...
def _stdcp_one_replicate(args):
    ...
    # Match suite: one shared RNG permutes calib PUMAs (sorted) then the target.
    # Only permuting the target would desynchronize row order from GHCP/HCP.
...
def _result_dir_for_alpha(alpha: float) -> Path:
    ...
    matches = sorted(
        (SUITE_ROOT / "results").glob(f"true_marginal_permuted_rf_income_notrim*yoep2000_{tag}"),
...
def patch_suite_results(alpha: float, stdcp_detailed: pd.DataFrame) -> Path:
    """Replace placeholder Std-CP rows in the finished suite detailed CSV."""
    tag = f"alpha{int(round(alpha * 100)):02d}"
    ...
    backup = detailed_path.with_suffix(".csv.bak_nostdcp")
    ...
    print(
        f"  Std-CP finite widths: "
        f"{int(np.isfinite(std['width'].to_numpy(dtype=float)).sum())}/{len(std)}",
        flush=True,
    )
    return detailed_path
...
def main() -> None:
    ...
    p.add_argument(
        "--patch_only",
        action="store_true",
        help="Only merge existing stdcp_studentized_*_detailed.csv into suite results",
    )
    args = p.parse_args()
    ...
    if not args.patch_only:
        print("Loading paper cohort (age 25–54, hours≥40, YOEP≥2000, exclude COW)...")
    ...
    for alpha in alphas:
        tag = f"alpha{int(round(alpha * 100)):02d}"
        if args.patch_only:
            path = OUT_ROOT / tag / f"stdcp_studentized_{tag}_detailed.csv"
            if not path.exists():

#!/usr/bin/env python3
"""Paired fixed-target simulation with gamma in {0, 6}."""
...
OUT_DIR = REPO_ROOT / "results_conditional" / "dgp"
PLOT_DIR = REPO_ROOT / "plots_conditional"
...
def generate_historical_groups(gamma: float, rng: np.random.Generator, use_poisson: bool = False) -> Tuple[np.ndarray, List[List[dict]]]:
    u_hist = rng.uniform(low=U_MIN, high=U_MAX, size=(K_HISTORICAL, DIMENSION))
    b_hist = rng.normal(loc=0.0, scale=gamma, size=K_HISTORICAL)
    if not use_poisson:
...
def generate_target_groups(gamma: float, n_groups: int, rng: np.random.Generator, use_poisson: bool = False) -> Tuple[np.ndarray, List[List[dict]]]:
    u_tgt = rng.uniform(low=U_MIN, high=U_MAX, size=(n_groups, DIMENSION))
    b_tgt = rng.normal(loc=0.0, scale=gamma, size=n_groups)
    if not use_poisson:
...
def run_one_experiment(exp_id: int, gamma: float, alpha: float) -> pd.DataFrame:
    use_poisson = POISSON_MODE
    seed = BASE_SEED + int(gamma * 1_000_000) + int(alpha * 10_000) + exp_id
    rng = np.random.default_rng(seed)

    u_hist, z_hist = generate_historical_groups(gamma, rng, use_poisson=use_poisson)
    u_target, z_target = generate_target_groups(gamma, N_TARGET_GROUPS_PER_EXPERIMENT, rng, use_poisson=use_poisson)
    ...
    for target_group in range(N_TARGET_GROUPS_PER_EXPERIMENT):
        ...
        for o in O_VALUES:
            ...
            rows.append(_row(gamma, alpha, exp_id, target_group, o, "Std-CP", interval_stdcp, y_true, y_target=y_true))
...
def run_randomization_stability_one_dataset(
    dataset_id: int, gamma: float, alpha: float, o: int
) -> Tuple[List[Dict], Dict]:
    use_poisson = POISSON_MODE
    seed = BASE_SEED + 50_000_000 + int(gamma * 1_000_000) + int(alpha * 10_000) + dataset_id
    rng = np.random.default_rng(seed)
    u_hist, z_hist = generate_historical_groups(gamma, rng, use_poisson=use_poisson)
    u_target, z_target = generate_target_groups(gamma, 1, rng, use_poisson=use_poisson)
...
def _plot_two_panel_boxes(
    ...
    d = df[(df["gamma"] == gamma) & (df["alpha"] == alpha)].copy()
...
def plot_requested_sets(df_all: pd.DataFrame, stability_summary: pd.DataFrame) -> None:
    ...
    for alpha_plot in ALPHA_VALUES:
        for gamma in GAMMA_VALUES:
            tag = f"{mode_tag}_gamma{int(gamma)}_{_alpha_tag(alpha_plot)}"
            ...
            d = df_all[(df_all["gamma"] == gamma) & (df_all["alpha"] == alpha_plot)].copy()
            fig, axes = plt.subplots(1, 2, figsize=(18, 6))
            ...
            stab = stability_summary[
                (stability_summary["gamma"] == gamma) & (stability_summary["alpha"] == alpha_plot)
            ].sort_values("o")
...
def main() -> None:
    ...
    for gamma in GAMMA_VALUES:
        for alpha in ALPHA_VALUES:
            args = [(exp_id, gamma, alpha) for exp_id in range(N_EXPERIMENTS)]
            print(f"[Main] Running experiments for gamma={gamma}, alpha={alpha} ({len(args)} jobs)")
    ...
    target_uniques = (
        df_all.groupby(["gamma", "alpha", "experiment", "target_group"], as_index=False)["y_target"]
        .nunique()
    ...
    summary_path = OUT_DIR / f"{mode_tag}_summary_by_gamma_alpha_o_method.csv"
    summary.to_csv(summary_path, index=False)

    exp_summary = (
        df_all.groupby(["gamma", "alpha", "experiment", "o", "method"], as_index=False)
        .agg(
            coverage_mean=("coverage", "mean"),
            width_mean=("width", "mean"),
            width_median=("width", "median"),
    ...
    for gamma in GAMMA_VALUES:
        for alpha in ALPHA_VALUES:
            for o in O_VALUES:
                args = [(dataset_id, gamma, alpha, o) for dataset_id in range(N_RANDOMIZATION_DATASETS)]
                print(f"[Stability] Running gamma={gamma}, alpha={alpha}, o={o} ({len(args)} datasets)")
                with Pool(N_WORKERS) as pool:
                    out_count = 0
                    for reruns, ds_row in pool.imap_unordered(run_stability_wrapper, args):
                        ...
                        if out_count % 10 == 0 or out_count == len(args):
                            ...
                            print(
                                ...
                                f"for gamma={gamma}, alpha={alpha}, o={o}"
                            )
    ...
    stability_summary = (
        df_stability_dataset.groupby(["gamma", "alpha", "o"], as_index=False)
        .agg(

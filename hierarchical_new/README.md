...
Test coverage at specific quantiles of a key covariate (income or baseline SBP).

- **Setup**:
  1. For each test group, identify observations at covariate percentiles p ∈ {0, 25, 50, 75}:
     - Sort by covariate (income or SBP) to find which observations are at these percentiles
     - Map back to original observation indices
  2. For each percentile observation:
     - Use all observations before that index as history
     - Predict the observation at that percentile
     - Record coverage indicator
  3. Aggregate coverage across all test groups and percentiles
- **Coverage**: Reported per percentile and overall
- **Use case**: Covariate-conditional coverage guarantees
- **Note**: Original (unsorted) data order is used for all HCP method calculations to preserve exchangeability

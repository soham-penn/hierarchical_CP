...
def run_marginal_experiment_one_clinic(
    ...
    if n_test < 1:
        print(f"  Warning: Test clinic {test_clinic} has no observations, skipping")
        return pd.DataFrame()
...
def main():
    ...
    detailed_path = output_dir / 'bp_detailed.csv'
    ...
    # Save summary
    summary_path = output_dir / 'bp_summary.csv'
    summary.to_csv(summary_path)
    print(f"   Summary saved to {summary_path}")

    # Generate markdown table
    md_path = output_dir / 'bp_summary.md'
    ...
    plot_path = plots_dir / 'bp_results.png'
    ...
    print(f"  - bp_detailed.csv ({len(full_results)} predictions)")
    print(f"  - bp_summary.csv (method summaries)")
    print(f"  - bp_summary.md (formatted results)")
    print(f"\nPlots saved to {plots_dir}/:")
    print(f"  - bp_results.png (coverage and width plots)")

...
def generate_calibration_data(number_groups, lambda_Poisson, dgp_specification):
    ...
    for j in range(number_groups):
        ...
        # Get U-dependent covariance matrix for X
        Sigma_X = dgp_specification['covariance_X'](d, Uj)
        ...
        for i in range(Nj):
            ...
            sd_Y = dgp_specification['noise_sd_Y'](x, Uj)
...
def generate_test_group(lambda_Poisson, dgp_specification, o_observed, fixed_U=None):
    """
    ...
    fixed_U : ndarray of shape (d,) or None
        If provided, use this as the group-level covariate instead of
        drawing from the prior.  Used by the conditional-coverage
        experiment to fix U to a grid value.
    ...
    # Generate or fix group-level covariate
    ...
    # Get U-dependent covariance matrix for X
    Sigma_X = dgp_specification['covariance_X'](d, U_test[0, :])
    ...
    for i in range(N_test):
        ...
        sd_Y = dgp_specification['noise_sd_Y'](x, U_test[0, :])
        y = np.random.normal(loc=mu_Y, scale=sd_Y)

#!/usr/bin/env python3
"""Heterogeneous DGP: X ~ N(μ(U), Σ(U)), Y|X ~ N(||X||, 1^T X 1).

This DGP creates MORE heterogeneity across groups:
- X ~ N(μ(U), Σ(U)) where μ(U) = U² and Σ(U) = (1-ρ)diag(U) + ρ·1·1^T
- Y | X ~ N(||X||, 1^T X 1)

The conditional distribution of Y given X has:
- Mean = ||X|| (Euclidean norm, highly nonlinear)
- Variance = sum(X) (heteroscedastic)

This makes groups with different U values very different, testing whether
within-group training helps.
"""
...
O_VALUES = [0, 20, 50, 100, 200, 300, 500]
TARGET_INDEX = 500
STDCP_MIN_O = 10
...
ALPHA_SELECTION = 0.5
NUMBER_GROUPS_K = 20
NUMBER_EXPERIMENTS = 20  # Reduced for speed with large o
NUMBER_TEST_GROUPS = 50  # Reduced for speed with large o

# Ridge regression regularization parameter
RIDGE_ALPHA = 1.0

TAG = "fixedN50_K20_d5_heterogeneous_dgp_ridge_o_upto_500"
...
def draw_group_heterogeneous(u_vec, n_obs, rho):
    ...
    Returns list of {'X': x_vec, 'Y': y_val} dicts.
    """
    mu_x = x_mean(u_vec)
    ...
    for _ in range(n_obs):
        ...
        # Compute Y | X distribution
        mean_y = np.linalg.norm(x)  # ||X||
        var_y = max(0.1, np.sum(x))  # 1^T X 1, with floor at 0.1 for stability
        std_y = np.sqrt(var_y)
...
def _local_ridge_model(z_group, m_j, ridge_alpha=RIDGE_ALPHA):
    """Fit local Ridge regression on first m_j observations: Y ~ 1 + X.

    Uses Ridge regression instead of OLS to reduce overfitting when m_j is small.
    """
    if m_j < 2:  # Need at least 2 points
        return None
    ...
    try:
        ...
        # Don't penalize intercept (first column)
        p = X_aug.shape[1]
...
def _merge_mu(mu_global, mu_local, m_j, s_size, is_valid):
    """Merge global and local predictions with λ = m_j / (s_size + m_j)."""
...
def _compute_donor_hcp_interval(
    ...
    alpha_selection,
    mu_method,
    use_local_ols,  # True or False
    test_index_target=None,
):
    """Compute D-HCP interval with or without local OLS."""
    K = len(Z_calibration)
    ...
    if len(S_tilde) == 0:
        ...
        m_test = _m_j(o_observed=o_observed, n_prime=o_observed, use_within=use_local_ols)
        local_est_test = _local_ridge_model(Z_test, m_test) if use_local_ols else None
    ...
    for j in S_cal:
        N_j_prime = int(N[j])
        m_j = _m_j(o_observed=o_observed, n_prime=N_j_prime, use_within=use_local_ols)
        if N_j_prime <= m_j:
            continue

        local_est_j = _local_ridge_model(Z_calibration[j], m_j) if use_local_ols else None
...
def _run_one_experiment(mu_baseline, mu_hcp):
    """Run one experiment: generate data and compute intervals for all methods."""
    ...
    rows = []
    for o in O_VALUES:
        cov_none, cov_ols, cov_stdcp, cov_hcp = [], [], [], []
        wid_none, wid_ols, wid_stdcp, wid_hcp = [], [], [], []
        inf_none = inf_ols = inf_stdcp = inf_hcp = 0
...
def _summarize(results_df):
    ...
    methods = [
        ...
        ('D-HCP (Ridge)', 'coverage_donor_ols', 'width_donor_ols', 'infinite_donor_ols'),
...
def main():
    ...
    print('=' * 100)
    print('HETEROGENEOUS DGP: X ~ N(μ(U), Σ(U)), Y|X ~ N(||X||, 1^T X 1)')
    print('=' * 100)
    ...
    print(f'\nComparing D-HCP without vs with local Ridge training (alpha={RIDGE_ALPHA}).\n')

    results = _run_experiments()
    raw_csv = dir_files / 'results_heterogeneous_dgp.csv'
    results.to_csv(raw_csv, index=False)
    print(f'\n  Saved: {raw_csv}')

    summary = _summarize(results)
    summ_csv = dir_summ / 'summary_heterogeneous_dgp.csv'

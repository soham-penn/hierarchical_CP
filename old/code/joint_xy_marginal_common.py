...
def plot1_side_by_side_dhcp_vs_hcp(results_o, alpha, save_path, o_max=20):
    """Coverage/width side-by-side with GHCP(o) and HCP."""
    ...
    for ax, metric, ylabel in [
        ...
        if metric == "coverage":
            ...
            ax.set_title("Coverage: GHCP vs HCP", fontsize=24, fontweight="bold")
        else:
            ax.set_title("Width: GHCP vs HCP", fontsize=24, fontweight="bold")

    # Simplified legend: show 4 color boxes + GHCP label in center, and HCP
    legend_handles = []
...
def plot_metric_dhcp_stdcp_hcp_by_o(results_o, alpha, metric, save_path, o_max=40):
    """For each o, draw three boxes: GHCP(o), Std-CP(o), and fixed HCP."""
    o_vals = sorted(results_o["test_sample_size_o"].unique())
    if o_max is not None:
        o_vals = [o for o in o_vals if o <= o_max]
    if len(o_vals) == 0:
    ...
    if metric == "coverage":
        ...
        ax.set_title("Coverage by o: GHCP vs Std-CP vs HCP", fontsize=24, fontweight="bold")
    else:
        ax.set_ylabel("Width", fontsize=24)
        ax.set_title("Width by o: GHCP vs Std-CP vs HCP", fontsize=24, fontweight="bold")

    legend_handles = [
        mpatches.Patch(facecolor="#777777", edgecolor="#777777", alpha=0.75, label="GHCP (color encodes o)"),
...
def plot_metric_with_all_baselines(results_o, alpha, metric, save_path):
    """Coverage/width with GHCP(o) and all baseline methods."""
    o_vals = sorted(results_o["test_sample_size_o"].unique())
    ...
    if metric == "coverage":
        ...
        ax.set_ylabel("Coverage", fontsize=24)
        ax.set_ylim(0.6, 1.05)
        ax.set_title("Coverage: GHCP vs baselines", fontsize=24, fontweight="bold")
    else:
        target = None
        ax.set_ylabel("Width", fontsize=24)
        ax.set_title("Width: GHCP vs baselines", fontsize=24, fontweight="bold")
...
def generate_requested_plots_from_results(results_o, alpha, dir_plots):
    ...
    # (2) For each o (up to 40): GHCP, Std-CP(o), and fixed HCP.
